...
def process_video_frame(video_frame):
    ...
    # post processing
    coord_hw_crop, center_ords = detect_keypoints(np.squeeze(keypoints_scoremap_v))
    coord_hw = trafo_coords(coord_hw_crop, center_v, scale_v, 256)
    center_ords = trafo_coords(center_ords, center_v, scale_v, 256)
    print("center_ord",center_ords)
    ...
    if center_ords[0, 1]> 200 or center_ords[0, 1] < 120:
        speed = fast_speed
    else:
        speed = slow_speed
    print("speed",speed)
    ...
    # 设置相机位置
    pybullet.resetDebugVisualizerCamera(radius, roll+90, -30, [x, y, 1])
...
if __name__ == '__main__':
    ...
    cv2.resizeWindow("Frame", 320,240)    # 设置长和宽

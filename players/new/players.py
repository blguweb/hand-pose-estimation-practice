...
SAM_URDF_PATH = os.path.join(pd.getDataPath(), "samurai.urdf")
...
class Games(object):
    ...
    def __init__(self):
        ...
        # pybullet.connect(pybullet.GUI, options="--minGraphicsUpdateTimeMs=0 --mp4=\"pybullet_grasp.mp4\" --mp4fps="+str(fps) )
        pybullet.setPhysicsEngineParameter(maxNumCmdPer1ms=1000)
        pybullet.resetDebugVisualizerCamera(cameraDistance=3, cameraYaw=-math.pi/2, cameraPitch= -30, cameraTargetPosition=[0.7,0,0])
    ...
    def load_robot(self):
        ...
        orn = [0.,0.,math.pi]
        pybullet.loadURDF(SAM_URDF_PATH, [0.4, 0, -0.97],useFixedBase=True)
        # table = pybullet.loadURDF(TABLE_URDF_PATH, [0.5, 0, -0.9450], globalScaling=1.5)
        table = pybullet.loadURDF(TABLE_URDF_PATH, [0.4, 0, -0.92], globalScaling=1.5)
        ...
        panda2 = pybullet.loadURDF(PANDA_URDF_PATH, [1.0, 0, 0], [0, 0, 1, 1], useFixedBase=True, flags=flags)
        ...
        for i in range(self.lego_count):
            ...
            # print("offset",offset)
            legos.append(pybullet.loadURDF(LEGO_URDF_PATH,np.array([0.5, 0, 0])+offset,[0, 0, 0, 1], flags=flags,globalScaling=0.8))
    ...
    def start_game(self):
        ...
        pybullet.addUserDebugText(
            text="Game rules: There are a total of 20 legos on the table. After a text tip has appeared, each player gives the gesture",
            textPosition=[-2, 0, 1.2],
            textColorRGB=[0, 0, 0],
            textSize=textsize,
            lifeTime=time
        )
        pybullet.addUserDebugText(
            text="indicates the number of legos they would take. A maximum of 5 can be taken at a time. The player who has taken all",
            textPosition=[-2, 0, 0.9],
            textColorRGB=[0, 0, 0],
            textSize=textsize,
            lifeTime=time
        )
        pybullet.addUserDebugText(
            text="the Lego at the end wins. The game starts with the left hand by default.",
            textPosition=[-2, 0, 0.6],
            textColorRGB=[0, 0, 0],
            textSize=textsize,
            lifeTime=time
        )
    ...
    def update_state(self):
        ...
        if len(keys)>0:
            for k,v in keys.items():
                if v&pybullet.KEY_WAS_TRIGGERED:
                    if (k==ord('1')):
                        self.state = 1
                    if (k==ord('2')):
                        self.state = 2
                    ...
                    if (k==ord('6')):
                        self.state = 6
                if v&pybullet.KEY_WAS_RELEASED:
                        self.state = 0
    
    def step(self,panda,destination_x,destination_y,side):
        if self.state==6:
            self.finger_target = 0.01
        if self.state==5:
            self.finger_target = 0.04 
        pybullet.submitProfileTiming("step")
        self.update_state()
        ...
        if self.state==1 or self.state==2 or self.state==3 or self.state==4 or self.state >=7:
            ...
            if side == "left":
                ...
                orn = pybullet.getQuaternionFromEuler([0,math.pi,-math.pi])
            pybullet.submitProfileTiming("IK")
...
class PandaSimAuto(Games):
    def __init__(self):
        ...
        self.state_durations=[0.5,0.5,0.5,1,0.5,0.5,0.5]
        for i in range(2):

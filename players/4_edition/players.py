...
class Games(object):
    ...
    def __init__(self):
        ...
        self.left_panda, self.right_panda = self.load_robot()
        ...
        self.h = 0
        self.flag = False
        self.keypoints = np.zeros((2, 21,2))
        self.frame = None
        self.t = 0.
    ...
    def load_lego(self):
        ...
        if self.lego_count == -1:
            print("don't get the legos!")
        legos=[]
        legox = 0
        legoy = 0
        for i in range(self.lego_count):
            ...
            print("offset",offset)
            legos.append(pybullet.loadURDF(LEGO_URDF_PATH,np.array([0.35, -0.15, 0])+offset,[0, 0, 0, 1], flags=flags,globalScaling=0.8))
            pybullet.changeVisualShape(legos[i],-1,rgbaColor=[random.random(),random.random(),random.random(),1])
            self.finish = 1 ## total number load finished!
        return legos
    ...
    def start_game(self):
        ...
        pybullet.addUserDebugText(
            text="Game rules: There are some legos on the table. After a text",
    ...
    def legos_indicate(self,step):
        print("right",step)
        pybullet.addUserDebugText(
            text="The total number of legos is "+str(step)+" .",
            textPosition=[-1, 0, 1],
            textColorRGB=[0, 0, 0],
            textSize=1.0,
            lifeTime=2
